...
app.post('/login', async (req, res) => {
    ...
    const isAdmin = email == 'mahanth@gmail.com' && password == 'mahanth@1234';
    const isMatch = await bcrypt.compare(password, user.password);
    if (!isMatch) {
        return res.status(401).json({ message: 'Invalid email or password' });
    }
...
app.get('/api/feedback', async (req, res) => {
  try {
    const feedbacks = await models.Feedback.find().sort({ createdAt: -1 });
    res.json(feedbacks);
  } catch (err) {
    res.status(500).json({ error: 'Server error' });
  }
...
app.post('/seller-register', async (req, res) => {
    try {
        ...
        const sellerExists = await models.Sellers.findOne({ $or: [{ username }, { email }] });
        if (sellerExists) {
            return res.status(400).json({ message: 'Username or email already exists' });
        }
        const salt = await bcrypt.genSalt(10);
        const hashedPassword = await bcrypt.hash(password, salt);
        const newSeller = new models.Sellers({
            firstname,
            lastname,
            username,
            email,
        ...
        console.log(error);
        return res.status(500).json({ error: 'An error occurred during seller registration' });
    }
});
...
app.post('/seller-login', async (req, res) => {
    const { email, password } = req.body;
    const seller = await models.Sellers.findOne({ email });
    if (!seller) {
        return res.status(401).json({ message: 'Invalid email or password' });
    }
    ...
    // Generate a JWT token for seller
    const sellerToken = jwt.sign({ sellerId: seller._id }, 'SELLER_SECRET_TOKEN', { expiresIn: '30d' });
    res.json({ seller, sellerToken });
});

// Add product (associate with seller)
app.post('/seller/add-product', async (req, res) => {
    try {
        const { productname, description, price, image, category, countInStock, rating, sellerId } = req.body;
        if (!productname || !description || !price || !image || !category || !countInStock || !rating || !sellerId) {
            return res.status(400).send({ message: 'Missing required fields' });
        }
        const product = new models.Product({
            productname,
...
// Get all products (for seller dashboard, can filter by sellerId)
app.get('/seller/products/:sellerId', async (req, res) => {
    try {
        const { sellerId } = req.params;
        const products = await models.Product.find({ sellerId });
...
// Get all products (for seller to see all products)
app.get('/seller/all-products', async (req, res) => {
    try {
        const products = await models.Product.find();
        res.status(200).json(products);
    } catch (error) {
